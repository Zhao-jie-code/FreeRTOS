...
static TaskHandle_t  g_xTaskHandlelCar1;
static TaskHandle_t  g_xTaskHandlelCar2;
static TaskHandle_t  g_xTaskHandlelCar3;
	...
	while (1)
	{
		...
		//if (idata.val == pcar->control_key)
		{
			if (pcar->x < g_xres - CAR_LENGTH)
			{
				...
				{
					/* 发出任务通知*/
					xTaskNotifyGive(g_xTaskHandlelCar2);
                    xTaskNotify(g_xTaskHandlelCar3, 100, eSetValueWithOverwrite);
					vTaskDelete(NULL);
				}
			}
		}
...
static void Car2Task(void *params)
{
    ...
    /*等待事件组*/
//	xEventGroupWaitBits(g_xEventCar, 0x01, pdTRUE, pdFALSE, portMAX_DELAY);
    /*等待任务通知*/
...
//				mdelay(50);
				...
				if (pcar->x == g_xres - CAR_LENGTH)
				{
                    ...
                    /* 设置事件组 ：bit0*/
					xEventGroupSetBits(g_xEventCar, 0x10);
...
//	xEventGroupWaitBits(g_xEventCar, 0x11, pdTRUE, pdTRUE, portMAX_DELAY);
    /*等待任务通知*/
    do
    {
...
//				mdelay(50);
                ...
                if(xEventGroupWaitBits(g_xEventCar, 0x10, pdTRUE, pdTRUE,pdTRUE))
                {
                    vTaskDelete(NULL);
                }
			}
...
#endif
    xTaskCreate(Car1Task, "car1", 128, &g_cars[0], osPriorityNormal, &g_xTaskHandlelCar1);
    xTaskCreate(Car2Task, "car2", 128, &g_cars[1], osPriorityNormal, &g_xTaskHandlelCar2);
    xTaskCreate(Car3Task, "car3", 128, &g_cars[2], osPriorityNormal, &g_xTaskHandlelCar3);	
}
