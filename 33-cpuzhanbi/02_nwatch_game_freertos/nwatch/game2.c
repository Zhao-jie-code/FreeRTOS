...
//        char *pcTaskGetName(xTaskHandle);
...
#endif
    xTaskCreate(Car1Task, "car1", 80, &g_cars[0], osPriorityNormal, NULL);
    xTaskCreate(Car2Task, "car2", 80, &g_cars[1], osPriorityNormal+2, &g_TaskHandleCar2);
    xTaskCreate(Car3Task, "car3", 85, &g_cars[2], osPriorityNormal+2, &g_TaskHandleCar3);
//    xTaskHandle = xTaskGetCurrentTaskHandle();
////        char *pcTaskGetName(xTaskHandle);
//    FreeNum = uxTaskGetStackHighWaterMark( xTaskHandle );
//    printf("FreeStack of Task %s : %d\n\r",  pcTaskGetName(xTaskHandle), FreeNum);    
