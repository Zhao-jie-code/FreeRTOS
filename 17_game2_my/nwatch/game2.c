...
//QueueHandle_t xQueuegame2;//创建在开头，在执行CarTask的时候就不对了，因为三个任务调用CarTask就只会创建一个队列，放在函数里面创建队列，每次就能创建3个队列
...
static void CarTask(void *params)
{
    ...
    /*创建自己的队列*/
    QueueHandle_t xQueuegame2 = xQueueCreate( 10, sizeof(struct ir_data));
    
    /*注册队列*/
    RegisterQueueHandle(xQueuegame2);
    ...
    {
        /*读取按键值:都队列*/
        xQueueReceive(xQueuegame2, &idata, portMAX_DELAY);
        /*控制汽车往右移动*/
        if(idata.val == pcar->contro_key)
        {
            if(pcar->x < g_xres - 15)
            {
                /*隐藏汽车*/
