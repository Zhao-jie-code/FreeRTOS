...
static QueueSetHandle_t g_xQueueIR;/*红外句柄*/
static QueueSetHandle_t g_xQueueRotary;/*编码器句柄*/
static QueueSetHandle_t g_xQueueMPU6050;/*编码器句柄*/

QueueHandle_t g_xQueuePlatfrom;/*x表示结构体，g-表示全局变量，挡球板队列句柄*/
...
static void platform_task(void *params)
{
    ...
    while (1)
    {
        ...
        xQueueReceive(g_xQueuePlatfrom, &idata, portMAX_DELAY);
...
static void ProcessIRData(void)
{
    ...
    static struct input_data inputdata;
    ...
    if(idata.val == 0xe0)
    {
        ...
        inputdata.val = UPT_MOVE_LEFT;
    }
    else if(idata.val == 0x90)
    {
        inputdata.dev = idata.dev;
        inputdata.val = UPT_MOVE_RIGHT;
    }
     else if(idata.val == 0x00)
    {
        /*保持不变*/
    }
    else
    {
        inputdata.dev = idata.dev;
        inputdata.val = UPT_MOVE_NONE;
    }
    
    xQueueSend(g_xQueuePlatfrom, &inputdata, 0);
}
...
static void ProcessMPU6050Data(void)
{
    ...
    xQueueReceive(g_xQueueMPU6050, &idata, 0);
    
    if(idata.angle_x > 90 )
    {
        inputdata.val = UPT_MOVE_LEFT;
    }
    else if(idata.angle_x < 90 )
    {
        inputdata.val = UPT_MOVE_RIGHT;
    }
    else
...
static void InputTask(void *params)//编码器控制挡板移动
{
    struct rotary_data rdata;
    struct input_data idata;
...
{		
    ...
    /*创建队列，创建队列集，创建输入任务Input*/
    g_xQueuePlatfrom = xQueueCreate( 10, sizeof(struct input_data));
    g_xQueueSetInput =  xQueueCreateSet( 30);
    
    g_xQueueIR = GetQueueIR();
    g_xQueueRotary = GetQueueRotary();
    g_xQueueMPU6050 = GetQueueMPU6050();
    
    xQueueAddToSet( g_xQueueIR, g_xQueueSetInput);/*将队列加入队列集*/
